				...
				Diagnosis.findOne({ diagnosisID })
					.then((result) => {
						if (result.length == 0) {
							...
							async function getMedicName() {
								try {
									if (result.medicValidatorID == ''){
										return 'N/A';
									}else{
										let resultUser = await User.findOne({ userID: result.medicValidatorID });
										if (resultUser.length == 0) return 'N/A';
										else return resultUser.name;
									}
								} catch (e) {
									console.log('error', e);
									return 'N/A';
							...
							getMedicName().then((medicValidatorName) => {
								const content = `
								<!doctype html>
								<html>
								...
								<div class="my-5 page" size="A4">
									...
									<section class="details">
										...
										<div style="width:19cm; height:4cm; border-bottom:2px solid #ddd;"></div>
									</section>
